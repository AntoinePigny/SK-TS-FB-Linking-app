<script lang="ts">
   ...
   $: isValid = username?.length > 2 && username.length < 16 && regex.test(username)
   ...
   async function confirmUsername() {
      ...
      batch.set(doc(db, 'users', $user!.uid), {
         ...
         published: true,
         bio: 'Je ne suis pas Zidane',
         links: [
            {
               title: 'Test link',
               url: 'https://kriss.kross',
               icon: 'custom',
            },
         ],
      })
   ...
   {:else}
      <form class="w-2/5" on:submit|preventDefault={confirmUsername}>
         ...
         <div class="my-4 min-h-16 px-8 w-full">
            {#if loading}
               <p class="text-info">Checking availability of @{username}...</p>
            {/if}
